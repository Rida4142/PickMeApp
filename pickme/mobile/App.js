...
import { api, session, setToken } from './api';
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      splashTimer = setTimeout(() => mounted && setScreen((current) => current === 'splash' ? 'onboarding' : current), 1500);
    })();
    return () => { mounted = false; if (splashTimer) clearTimeout(splashTimer); };
  }, []);

  const go = setScreen;
  const requireUser = () => { if (!me) { go('auth'); return false; } return true; };
  const search = async () => {
  ...
  const finishAuth = (token, user) => { setToken(token); setMe(user); session.save(token, user); go('home'); };

  if (screen === 'splash') return <View style={[s.fill, { backgroundColor: C.y, justifyContent: 'center', padding: 30 }]}><Text style={s.logo}>PickMe</Text><Text style={s.splashTagline}>Same route.{ '\n' }Better together.</Text><CarIllustration style={s.splashIllustration} /><Text style={s.splashHint}>Share the ride. Keep more.</Text></View>;
  if (screen === 'onboarding') return <OnboardingScreen page={onboardingPage} setPage={setOnboardingPage} done={() => go('auth')} />;
  ...
  if (screen === 'auth') return <AuthScreen onDone={finishAuth} back={() => go('home')} say={say} />;
  ...
  return <SafeAreaView style={s.fill}><ScrollView contentContainerStyle={s.pad} keyboardShouldPersistTaps="handled">
    ...
    {screen === 'profile' && (me ? <ProfileScreen me={me} logout={logout} go={go} setForm={setForm} /> : <Guest go={go} />)}
